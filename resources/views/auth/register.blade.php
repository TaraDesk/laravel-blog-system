<x-app.layout>
    <x-slot:title>
        Register | StoryHub
    </x-slot:title>

    <div class="w-screen min-h-screen flex items-center justify-center bg-gray-50 px-4 sm:px-6 lg:px-8">
        <div class="relative py-3 sm:max-w-xl sm:mx-auto">
            <div class="min-h-96 px-8 py-6 mt-4 text-left bg-white rounded-xl shadow-lg">
                <div class="flex flex-col justify-center items-center h-full select-none">
                    <div class="flex flex-col items-center justify-center gap-2 mb-8">
                        <p class="text-lg font-semibold text-gray-800">Create your account</p>
                        <span class="text-sm text-center text-gray-500">
                        Sign up to share stories, follow authors, and join the conversation.
                        </span>
                    </div>
            
                    <form action="{{ route('register.store') }}" method="POST" class="w-full">
                        @csrf

                        <div class="flex gap-4">
                            <x-form.input name="name" title="Full Name" icon="fa-user" type="text" />
                            <x-form.input name="username" title="Username" icon="fa-user" type="text" />
                        </div>
                        <x-form.input name="email" title="Email Address" icon="fa-envelope" type="email" />
                        <x-form.input name="password" title="Password" icon="fa-lock" type="password" />
                        <x-form.input name="password_confirmation" title="Password Confirmation" icon="fa-lock" type="password" />
                        
                        <x-form.button>
                            Register
                        </x-form.button>
                    </form>

                    <p class="mt-4 text-sm text-gray-600">
                        Already have an account?
                        <a href="{{ route('login') }}" class="text-blue-600 hover:underline font-medium">Sign in here</a>
                    </p>
                </div>
            </div>
        </div>
    </div>

    @if ($errors->any())
        <x-alerts.error :error="$errors->all()" />
    @endif
</x-app.layout>